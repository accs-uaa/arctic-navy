# -*- coding: utf-8 -*-
# ---------------------------------------------------------------------------
# Train and predict salt-killed model
# Author: Timm Nawrocki
# Last Updated: 2026-03-05
# Usage: Must be executed in a Python 3.12+ installation.
# Description: 'Train and predict salt-killed model' trains and predicts a random forest model of salt-killed tundra for the North Slope at 10 m resolution.
# ---------------------------------------------------------------------------
...
acc_output = os.path.join(output_folder, f'saltkilled_acc.txt')
...
# Define input data
training_data = ee.FeatureCollection(f'{asset_path}/{export_prefix}/NorthSlope_SaltKilled_Covariates')
...
# Define a function to calculate percentage binary accuracy
def calc_accuracy(feature_collection, actual_prop, pred_prop):
    # Create an error matrix to assess accuracy
    error_matrix = feature_collection.errorMatrix(actual_prop, pred_prop)
    # Extract overall accuracy and multiply by 100 for percentage
    accuracy_pct = error_matrix.accuracy().multiply(100)
    return accuracy_pct
...
# Train Model
print(f'Training and evaluating classifier...')
start_time = time.time()

# Train a classifier
print('\tTraining classifier on train partition...')
classifier = ee.Classifier.smileRandomForest(numberOfTrees=100).train(
    features=training_sample,
    classProperty='saltkilled',
    inputProperties=band_names
)

# Assess validation accuracy
print('\tPredicting classifier on test partition...')
val_predicted = validation_sample.classify(classifier, 'predicted_class')
val_acc = calc_accuracy(val_predicted, 'saltkilled', 'predicted_class')
...
print(f'\tValidation ACC:   {val_acc.getInfo():.2f}%')
...
for area_path in [icycape_path, kuparuk_path, utqiagvik_path]:
    ...
    task_description = f'{area_name}-saltkilled-10m'
    file_name = f'{export_prefix}/{area_name}_SaltKilled_10m_3338'
