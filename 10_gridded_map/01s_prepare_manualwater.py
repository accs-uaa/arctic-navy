# -*- coding: utf-8 -*-
# ---------------------------------------------------------------------------
# Prepare manual water data
# Author: Timm Nawrocki
# Last Updated: 2026-01-24
# Usage: Execute in Python 3.9+.
# Description: "Prepare manual water data" extracts manually delineated water raster to common extent, mask, grid, and cell size.
# ---------------------------------------------------------------------------
...
input_folder = os.path.join(project_folder, 'Data_Input/ancillary_data/Manual_Water_10m/unprocessed')
output_folder = os.path.join(project_folder, 'Data_Input/ancillary_data/Manual_Water_10m/processed')

# Define input files
area_input = os.path.join(project_folder, 'Data_Input/ArcticCoastal_MapDomain_10m_3338.tif')
water_input = os.path.join(input_folder, 'Manual_Water_10m_3338.tif')

# Define output file
water_output = os.path.join(output_folder, 'ArcticCoastal_ManualWater_10m_3338.tif')
...
output_profile = water_raster.profile.copy()
...
with rasterio.open(water_output, 'w', **output_profile) as dst:
    ...
    for block_index, window in dst.block_windows(1):
        ...
        input_window = from_bounds(*window_bounds,
                                   transform=water_raster.transform).round_offsets().round_lengths()

        # Read block data
        area_block = area_raster.read(1, window=window)
        out_block = water_raster.read(1, window=input_window)
...
iteration_start = time.time()
with rasterio.open(water_output, 'r+') as dst:
