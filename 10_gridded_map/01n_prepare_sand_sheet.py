...
sand_input = os.path.join(input_folder, 'SandSheet_Generalized_10m_3338.tif')

# Define output file
sand_output = os.path.join(output_folder, 'ArcticCoastal_SandSheet_Generalized_10m_3338.tif')
...
output_profile = sand_raster.profile.copy()
...
with rasterio.open(sand_output, 'w', **output_profile) as dst:
    ...
    for block_index, window in dst.block_windows(1):
        ...
        input_window = from_bounds(*window_bounds,
                                   transform=sand_raster.transform).round_offsets().round_lengths()

        # Read block data
        area_block = area_raster.read(1, window=window)
        out_block = sand_raster.read(1, window=input_window)
...
iteration_start = time.time()
with rasterio.open(sand_output, 'r+') as dst:
