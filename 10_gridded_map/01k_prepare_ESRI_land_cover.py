# -*- coding: utf-8 -*-
# ---------------------------------------------------------------------------
# Prepare ESRI land cover data
# Author: Timm Nawrocki
# Last Updated: 2026-01-07
# Usage: Execute in Python 3.9+.
# Description: "Prepare ESRI land cover data" extracts ESRI land cover raster to common extent, mask, grid, and cell size.
# ---------------------------------------------------------------------------
...
input_folder = os.path.join(project_folder, 'Data_Input/ancillary_data/ESRI_LandCover_10m/unprocessed')
output_folder = os.path.join(project_folder, 'Data_Input/ancillary_data/ESRI_LandCover_10m/processed')

# Define input files
area_input = os.path.join(project_folder, 'Data_Input/ArcticCoastal_MapDomain_10m_3338.tif')
esri_input = os.path.join(input_folder, 'AlaskaYukon_ESRI_10m_3338.tif')

# Define output file
esri_output = os.path.join(output_folder, 'ArcticCoastal_ESRILandCover_10m_3338.tif')
...
output_profile = esri_raster.profile.copy()
...
with rasterio.open(esri_output, 'w', **output_profile) as dst:
    ...
    for block_index, window in dst.block_windows(1):
        ...
        input_window = from_bounds(*window_bounds,
                                   transform=esri_raster.transform).round_offsets().round_lengths()

        # Read block data
        area_block = area_raster.read(1, window=window)
        out_block = esri_raster.read(1, window=input_window)
...
iteration_start = time.time()
with rasterio.open(esri_output, 'r+') as dst:
...
attribute_output = esri_output + '.vat.dbf'
...
iteration_start = time.time()
with rasterio.open(esri_output) as esri_raster:
    # Find number of raster blocks
    window_list = []
    for block_index, window in esri_raster.block_windows(1):
        window_list.append(window)
    # Iterate processing through raster blocks
    count = 1
    progress = 0
    for block_index, window in esri_raster.block_windows(1):
        input_block = esri_raster.read(1, window=window, masked=True)
...
colormap_output = esri_output + '.clr'
