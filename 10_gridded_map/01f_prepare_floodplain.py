...
# Description: "Prepare floodplain data" extracts floodplain raster to common extent, mask, grid, and cell size.
# ---------------------------------------------------------------------------
...
input_folder = os.path.join(project_folder, 'Data_Input/ancillary_data/Floodplain_10m/unprocessed')
output_folder = os.path.join(project_folder, 'Data_Input/ancillary_data/Floodplain_10m/processed')

# Define input files
area_input = os.path.join(project_folder, 'Data_Input/ArcticCoastal_MapDomain_10m_3338.tif')
floodplain_input = os.path.join(input_folder, 'NorthernArctic_Floodplain_10m_3338.tif')

# Define output file
floodplain_output = os.path.join(output_folder, 'ArcticCoastal_Floodplain_10m_3338.tif')
...
output_profile = floodplain_raster.profile.copy()
...
with rasterio.open(floodplain_output, 'w', **output_profile) as dst:
    ...
    for block_index, window in dst.block_windows(1):
        ...
        input_window = from_bounds(*window_bounds,
                                   transform=floodplain_raster.transform).round_offsets().round_lengths()

        # Read block data
        area_block = area_raster.read(1, window=window)
        out_block = floodplain_raster.read(1, window=input_window)
