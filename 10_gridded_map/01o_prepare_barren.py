...
# Description: "Prepare barren data" extracts barren raster modified from Arctic Coastal Plain geomorphology to common extent, mask, grid, and cell size.
# ---------------------------------------------------------------------------
...
barren_input = os.path.join(input_folder,
                          'NorthernAlaska_CoastalPlain_Geomorphology_Barren_Buff50_10m_3338.tif')

# Define output file
barren_output = os.path.join(output_folder, 'ArcticCoastal_Barren_10m_3338.tif')
...
output_profile = barren_raster.profile.copy()
...
with rasterio.open(barren_output, 'w', **output_profile) as dst:
    ...
    for block_index, window in dst.block_windows(1):
        ...
        input_window = from_bounds(*window_bounds,
                                   transform=barren_raster.transform).round_offsets().round_lengths()

        # Read block data
        area_block = area_raster.read(1, window=window)
        out_block = barren_raster.read(1, window=input_window)
...
iteration_start = time.time()
with rasterio.open(barren_output, 'r+') as dst:
