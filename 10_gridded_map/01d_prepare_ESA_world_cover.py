# -*- coding: utf-8 -*-
# ---------------------------------------------------------------------------
# Prepare ESA world cover data
# Author: Timm Nawrocki
# Last Updated: 2026-01-06
# Usage: Execute in Python 3.9+.
# Description: "Prepare ESA world cover data" extracts ESA world cover raster to common extent, mask, grid, and cell size.
# ---------------------------------------------------------------------------
...
input_folder = os.path.join(project_folder, 'Data_Input/ancillary_data/ESA_WorldCover_10m/unprocessed')
output_folder = os.path.join(project_folder, 'Data_Input/ancillary_data/ESA_WorldCover_10m/processed')

# Define input files
area_input = os.path.join(project_folder, 'Data_Input/ArcticCoastal_MapDomain_10m_3338.tif')
esa_input = os.path.join(input_folder, 'AlaskaYukon_ESAWorldCover2_10m_3338.tif')

# Define output file
esa_output = os.path.join(output_folder, 'ArcticCoastal_ESAWorldCover2_10m_3338.tif')
...
output_profile = esa_raster.profile.copy()
...
with rasterio.open(esa_output, 'w', **output_profile) as dst:
    ...
    for block_index, window in dst.block_windows(1):
        ...
        input_window = from_bounds(*window_bounds,
                                   transform=esa_raster.transform).round_offsets().round_lengths()

        # Read block data
        area_block = area_raster.read(1, window=window)
        out_block = esa_raster.read(1, window=input_window)
...
iteration_start = time.time()
with rasterio.open(esa_output, 'r+') as dst:
...
attribute_output = esa_output + '.vat.dbf'
...
iteration_start = time.time()
with rasterio.open(esa_output) as esa_raster:
    # Find number of raster blocks
    window_list = []
    ...
    progress = 0
    for block_index, window in esa_raster.block_windows(1):
        input_block = esa_raster.read(1, window=window, masked=True)
...
colormap_output = esa_output + '.clr'
