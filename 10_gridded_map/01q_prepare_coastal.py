# -*- coding: utf-8 -*-
# ---------------------------------------------------------------------------
# Prepare coastal data
# Author: Timm Nawrocki
# Last Updated: 2026-01-08
# Usage: Execute in Python 3.9+.
# Description: "Prepare coastal data" extracts custom coastal raster to common extent, mask, grid, and cell size.
# ---------------------------------------------------------------------------
...
input_folder = os.path.join(project_folder, 'Data_Input/ancillary_data/Coastal_Zone_10m/unprocessed')
output_folder = os.path.join(project_folder, 'Data_Input/ancillary_data/Coastal_Zone_10m/processed')

# Define input files
area_input = os.path.join(project_folder, 'Data_Input/ArcticCoastal_MapDomain_10m_3338.tif')
coastal_input = os.path.join(input_folder, 'Coastal_Zone_10m_3338.tif')

# Define output file
coastal_output = os.path.join(output_folder, 'ArcticCoastal_Coastal_10m_3338.tif')
...
output_profile = coastal_raster.profile.copy()
...
with rasterio.open(coastal_output, 'w', **output_profile) as dst:
    ...
    for block_index, window in dst.block_windows(1):
        ...
        input_window = from_bounds(*window_bounds,
                                   transform=coastal_raster.transform).round_offsets().round_lengths()

        # Read block data
        area_block = area_raster.read(1, window=window)
        out_block = coastal_raster.read(1, window=input_window)
...
iteration_start = time.time()
with rasterio.open(coastal_output, 'r+') as dst:
...
# Explicitly close rasters
area_raster.close()
coastal_raster.close()
