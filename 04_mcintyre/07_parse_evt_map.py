...
dune_input = os.path.join(input_folder, f'{region}_Dunes_0.5m_3338.tif')
floodplain_input = os.path.join(input_folder, f'{region}_Floodplain_0.5m_3338.tif')
...
barrencor_input = os.path.join(input_folder, f'{region}_Correction_Barren_0.5m_3338.tif')
...
floodplain_raster = rasterio.open(floodplain_input)
watercor_raster = rasterio.open(watercor_input)
saltcor_raster = rasterio.open(saltcor_input)
barrencor_raster = rasterio.open(barrencor_input)
distcor_raster = rasterio.open(distcor_input)
beachcor_raster = rasterio.open(beachcor_input)
...
with rasterio.open(vegetation_output, 'w', **output_profile) as dst:
    ...
    for block_index, window in area_raster.block_windows(1):
        ...
        dune_block = read_raster_block(dune_raster, window_bounds)
        floodplain_block = read_raster_block(floodplain_raster, window_bounds)
        ...
        barrencor_block = read_raster_block(barrencor_raster, window_bounds)
        distcor_block = read_raster_block(distcor_raster, window_bounds)
        beachcor_block = read_raster_block(beachcor_raster, window_bounds)
        ...
        # Correct barren
        surficial_block = np.where((barrencor_block == 1) & (np.isin(surficial_block, [2, 3, 6])),
                                   4, surficial_block)

        # Correct salt-intruded
        surficial_block = np.where(((saltcor_block == 1) & (surficial_block != 1))
                                   | (surficial_block == 6),
                                   7, surficial_block)

        # Correct tidal marsh
        surficial_block = np.where((surficial_block == 5)
                                   & ((dryas_block >= 5) | (nerishr_block >= 3)),
                                   2, surficial_block)
        surficial_block = np.where((surficial_block == 5)
                                   & ((coastal_block != 1)),
                                   3, surficial_block)
        ...
        coastal_block = np.where((dryas_block >= 5) | (nerishr_block >= 3),
                                 0, coastal_block)

        # Correct wet
        surficial_block = np.where((surficial_block == 2)
                                   & ((water_block >= 4) | (sphagn_block >= 7))
                                   & (dsalix_block < 13)
                                   & (dryas_block < 8)
                                   & (erivag_block < 4),
                                   3, surficial_block)
        ...
        # Correct mesic
        surficial_block = np.where((surficial_block == 3)
                                   & ((dsalix_block >= 13) | (dryas_block >= 8) | (erivag_block >= 4)),
                                   2, surficial_block)
        ...
        # 159. Arctic Herbaceous & Shrub Coastal Dune
        ...
        out_block = np.where((out_block == 0)
                             & (np.isin(surficial_block, [2, 3, 4]))
                             & ((dryas_block >= 5) | (dsalix_block >= 10) | (nerishr_block >= 3))
                             & (erivag_block < 4)
                             & (barrencor_block != 1)
                             & (floodplain_block == 1),
                             166, out_block)
        ...
        # 167. Arctic Herbaceous Floodplain
        out_block = np.where((out_block == 0)
                             & ((surficial_block == 2) |
                                ((surficial_block == 4) & (bromos_block > 5)))
                             & (erivag_block < 4)
                             & (floodplain_block == 1),
                             167, out_block)

        # 165. Arctic Barren & Sparsely Vegetated Floodplain
        out_block = np.where((out_block == 0)
                             & (surficial_block == 4)
                             & (floodplain_block == 1),
                             165, out_block)
        ...
        #### FLOODPLAIN WET TYPES

        # 170. Arctic Freshwater Marsh
        out_block = np.where((out_block == 0)
                             & (np.isin(surficial_block, [3, 1]))
                             & (coastal_block != 1)
                             & (water_block >= 20)
                             & (wetsed_block >= 8)
                             & (sphagn_block < 4)
        ...
        # 171. Arctic Wet Meadow (Riverine/Mineral)
        out_block = np.where((surficial_block == 3)
                             & ((floodplain_block == 1)
                                | ((dune_block == 1) & (coastal_block != 1))),
                             171, out_block)
        ...
        # 174. Infrastructure
        out_block = np.where((infrastructure_block == 1),
                             174, out_block)

        # 173. Disturbed Vegetation
        out_block = np.where((infrastructure_block == 1)
                             & ((dryas_block + dsalix_block + erivag_block + wetsed_block) >= 10),
                             173, out_block)
