# -*- coding: utf-8 -*-
# ---------------------------------------------------------------------------
# Extract covariates for salt-killed tundra
# Author: Timm Nawrocki
# Last Updated: 2026-03-05
# Usage: Must be executed in a Python 3.12+ installation.
# Description: "Extract covariates for salt-killed tundra" extracts a suite of remotely sensed covariates to the
# ---------------------------------------------------------------------------
...
# Define input data
training_points = ee.FeatureCollection(f'{asset_path}/{export_prefix}/AKVEG_Water')
...
buffer_feature = training_points.map(lambda f: f.buffer(f.getNumber('plot_radius_m')))
...
# Revert back to points
training_data = training_data.map(lambda f: f.centroid())

# Create an export task for the sampling reduction
task = ee.batch.Export.table.toAsset(
    collection=training_data,
    description='akveg-water-covariates',
    assetId=f'{asset_path}/{export_prefix}/AKVEG_Water_Covariates'
)

# Initiate task
task.start()
print(f'Export task initiated.')
